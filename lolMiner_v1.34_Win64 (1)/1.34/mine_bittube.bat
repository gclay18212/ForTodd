...
:W10
"%MyVariable%" -a CR29-40 --pool !POOL! --user !WALLET! --pass !WORKER!  --watchdog exit !EXTRAPARAMETERS!
if %ERRORLEVEL% == 42 (
	timeout 10
	goto W10
)
...
:OtherW
"%MyVariable%" -a CR29-40 --pool !POOL! --user !WALLET! --pass !WORKER! --watchdog exit !EXTRAPARAMETERS! --nocolor
if %ERRORLEVEL% == 42 (
	timeout 10
	goto OtherW
)
